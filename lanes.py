...
def canny(image):
    gray = cv.cvtColor(image, cv.COLOR_RGB2BGR)
...
def region_of_interest(image):
    ...
    height = image.shape[0] #480
    polygons = np.array([(width//3, height//2), (width-(width//10), height//2), (width, height), (0, height)])
    mask = np.zeros_like(image)
    cv.fillPoly(mask, np.array([polygons], dtype=np.int64), height)#1024
